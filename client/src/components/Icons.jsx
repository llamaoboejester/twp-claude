...
export const ELEMENT_SRC = {
  whimsy:    '/icons/source/whimsy.png',
  edge:      '/icons/source/edge.png',
  nature:    '/icons/source/nature.png',
  tradition: '/icons/source/tradition.png',
  elegance:  '/icons/source/elegance.png',
  wild:      '/icons/source/wild.png',
};
...
export function ElementIcon({ element, size = 24, mode = 'stamp', className = '', style = {} }) {
...
export function ElementPip({ element, size = 14 }) {
  const tone = ELEMENT_TONE[element] || 'var(--ink-line-2)';
  return (
    <span
      style={{
        display: 'inline-flex',
...
export function Coin({ size = 22, value = null, style = {} }) {
  return (
    <span
      style={{
        display: 'inline-flex',
...
export function GiftIcon({ size = 18, color = 'var(--gift)' }) {
  return (
...
export function Spark({ size = 18, color = 'var(--accent)' }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" style={{ flex: '0 0 auto' }}>
      <path
...
export function Meeple({ size = 28, color = 'var(--ink)', outline = null }) {
  return (
...
export function EffortBox({ filled = false, size = 14 }) {
  return (
    <span
      style={{
        display: 'inline-flex',
...
export function WeatherIcon({ size = 16, color = 'var(--ink)' }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" style={{ flex: '0 0 auto' }}>
...
export function LockIcon({ size = 14, color = 'var(--ink-3)' }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" style={{ flex: '0 0 auto' }}>
...
export function FirstPlayerToken({ size = 28 }) {
  return (
    <span
      style={{
        display: 'inline-flex',
...
export function CategoryIcon({ category, size = 24, mode = 'tile', tone = null, style = {} }) {
  const slug = CATEGORY_SLUG[category];
  if (!slug) return null;
  const bgColor = tone || CATEGORY_TONE[category];
  const src = `/icons/category/${slug}.png`;
